...
const Voter = ({signupVoterFromExcel,sendVoters}) => {
  ...
  const handleClick = (event, email) => {
      if(event.target.checked){
        ...
        var index = newSelected.indexOf(event.target.value)
        if (index !== -1) {
          newSelected.splice(index, 1);
        }
      }
      setSelected(newSelected)
      sendVoters(newSelected)
}
  ...
  return (
    <div >
        <Page title="Voters Page | NEARVOTE">
          <Container>
                  ...
                  <Table >
                    ...
                    <TableBody>
                      {filteredUsers
                        ...
                        .map((row) => {
                          const { _id, username, firstName, lastName, email, isVerified, role } = row;
